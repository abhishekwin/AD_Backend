...
const manageData = async (transferdata) => {
    ...
    await EventManager.updateOne({ name: "launchPadCreatedEventsBsc" }, { lastcrontime: timestamp })
};

const launchPadCreatedEventsBsc = async () => {
    let transfereventDetails = await EventManager.findOne({ name: "launchPadCreatedEventsBsc" })
    let from = 0
    if (transfereventDetails) {
        from = transfereventDetails.lastcrontime;
    } else {
        await EventManager.create({ name: "launchPadCreatedEventsBsc", lastcrontime: 0 })
    }
