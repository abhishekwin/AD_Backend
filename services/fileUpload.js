...
const storeImageOnLocal = async (imageUrl, nft) => {    
    ...
    const filepath = await storeImage(imageURL, appDir+'/public/nft-image-upload/'+randumFileName+'.png', nft);
    const nftImages = {
      png:{},
      webp:{}
    }
...
async function getTokenUriData(tokenURIUrl) {
  try {
    let config = {
        headers: {
...
const getImageUrlFromTokenUri = async (tokenURIUrl) => {
    const result = await getTokenUriData(tokenURIUrl);
    if(result){
...
const uploadImageOnS3 = async (tokenURI) => {
    ...
    try{
        ...
        if(imageUrl){
            if(isNaN(imageUrl)){              
              return await storeImageOnLocal(imageUrl);              
            }else{
              return false
            }            
        }else{
